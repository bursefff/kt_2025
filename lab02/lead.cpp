...
public:
    ...
    void move(double tau) {
        ...
        z += vz * tau;
        t += tau;
        smth = cos(20*t - 0.5 * x - 0.5 * y);
        double sigma = 40.0;
        vy = - 70.0 * exp(-(x-63)*(x-63)/2 / sigma / sigma) * exp(-(y-100)*(y-100)/2/10.0/10.0);
        vx = 0.15 * (x - 63)* exp(-(x-63)*(x-63)/2 / sigma / sigma);
        vy += 25.0 * cos(50 * t) * cos(y) * exp(-0.5 * t);
    }
};
