...
t = 2 * ufl.cos(7 * x[0]) + 3 * ufl.exp(-((x[0]-1.0)**2 + (x[2]-1.0)**2 + (x[1]-1.0)**2)/0.1)
a = inner(grad(u), grad(v)) * dx
L = inner(t, v) * dx #+ inner(g, v) * ds
# -

f1 = fem.Function(V)
f1.interpolate(lambda x: 0.1 * np.exp(-((x[1]-1.0)**2 + (x[2]-1.0)**2)/0.1))

bc = fem.dirichletbc(f1, dofs=dofs)
